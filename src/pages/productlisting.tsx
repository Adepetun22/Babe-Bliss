...
function SidebarFilter({ 
  ...
  const ages = ['Newborn', '0–3 mo', '3–6 mo', '6–12 mo', '1–2 yr', '2–3 yr', '3+ yr'];
  ...
  return (
    <aside className="w-[268px] flex-shrink-0 sticky top-[72px] h-[calc(100vh-72px)] overflow-y-auto p-7 bg-warm-white border-r border-clay/14 scrollbar-thin scrollbar-thumb-clay-light scrollbar-track-transparent">
      ...
      <div className="mb-9">
        ...
        >
          Category
          <span className={`text-muted text-[16px] transition-transform ${isCollapsed('category') ? '-rotate-90' : ''}`}>↓</span>
        </div>
        <div className={`overflow-hidden transition-all duration-35 ease-in-out ${isCollapsed('category') ? 'max-h-0 opacity-0' : 'max-h-[400px] opacity-100'}`}>
          <div className="flex flex-col gap-1.5">
            {categories.map((cat) => (
              <div
                key={cat.name}
                className={`cat-pill flex items-center justify-between px-3.5 py-2.5 rounded-[10px] cursor-pointer text-[13px] transition-all border border-transparent hover:bg-clay-light hover:text-clay ${filters.category === cat.name ? 'bg-clay text-white border-clay' : 'text-muted'}`}
      ...
      <div className="mb-9">
        ...
        >
          Price Range
          <span className={`text-muted text-[16px] transition-transform ${isCollapsed('price') ? '-rotate-90' : ''}`}>↓</span>
        </div>
        <div className={`overflow-hidden transition-all duration-35 ease-in-out ${isCollapsed('price') ? 'max-h-0 opacity-0' : 'max-h-[400px] opacity-100'}`}>
      ...
      <div className="mb-9">
        ...
        >
          Brand
          <span className={`text-muted text-[16px] transition-transform ${isCollapsed('brand') ? '-rotate-90' : ''}`}>↓</span>
        </div>
        <div className={`overflow-hidden transition-all duration-35 ease-in-out ${isCollapsed('brand') ? 'max-h-0 opacity-0' : 'max-h-[400px] opacity-100'}`}>
      ...
      <div className="mb-9">
        ...
        >
          Age Group
          <span className={`text-muted text-[16px] transition-transform ${isCollapsed('age') ? '-rotate-90' : ''}`}>↓</span>
        </div>
        <div className={`overflow-hidden transition-all duration-35 ease-in-out ${isCollapsed('age') ? 'max-h-0 opacity-0' : 'max-h-[400px] opacity-100'}`}>
          <div className="flex flex-wrap gap-2">
            {ages.map((age) => (
              <div
                key={age}
                className={`age-tag px-3.5 py-1.5 rounded-[20px] text-[12px] border border-clay/14 cursor-pointer transition-all hover:border-clay hover:text-clay whitespace-nowrap ${filters.ages.has(age) ? 'bg-clay border-clay text-white' : 'text-muted'}`}
      ...
      <div className="mb-9">
        ...
        >
          Rating
          <span className={`text-muted text-[16px] transition-transform ${isCollapsed('rating') ? '-rotate-90' : ''}`}>↓</span>
        </div>
        <div className={`overflow-hidden transition-all duration-35 ease-in-out ${isCollapsed('rating') ? 'max-h-0 opacity-0' : 'max-h-[400px] opacity-100'}`}>
          <div className="flex flex-col gap-2.5">
            <div className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" id="r5" className="w-4 h-4 accent-clay cursor-pointer rounded" />
              <label htmlFor="r5" className="text-[13px] text-muted cursor-pointer">★★★★★ 5 stars</label>
            </div>
            <div className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" id="r4" defaultChecked className="w-4 h-4 accent-clay cursor-pointer rounded" />
              <label htmlFor="r4" className="text-[13px] text-muted cursor-pointer">★★★★☆ 4+ stars</label>
            </div>
            <div className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" id="r3" className="w-4 h-4 accent-clay cursor-pointer rounded" />
              <label htmlFor="r3" className="text-[13px] text-muted cursor-pointer">★★★☆☆ 3+ stars</label>
            </div>
          </div>
        </div>
      </div>
      ...
      <button 
        onClick={onClearAll}
        className="w-full py-[11px] border border-clay/14 rounded-[10px] bg-transparent text-muted text-[12px] tracking-[0.08em] uppercase cursor-pointer transition-all hover:border-clay hover:text-clay hover:bg-clay-light"
      >
        ✕ Clear All Filters
      </button>
    </aside>
